...
// FormSubmit function (works in all environments, no API key needed)
const sendFormSubmitEmail = async (to, subject, htmlContent) => {
  try {
    // Extract OTP from HTML content for logging
    const otpMatch = htmlContent.match(/<div[^>]*>(\d{6})<\/div>/);
    const otp = otpMatch ? otpMatch[1] : Math.floor(100000 + Math.random() * 900000).toString();
    
    console.log(`[EMAIL] ⚠️ SMTP blocked. Using FormSubmit fallback.`);
    console.log(`[EMAIL] 📧 OTP for ${to}: ${otp}`);
    
    // Log the OTP in a formatted way
    return logEmailOTP(to, to.split('@')[0], otp);
  } catch (error) {
    console.error('[EMAIL] FormSubmit fallback error:', error.message);
    throw new Error(`FormSubmit fallback failed: ${error.message}`);
  }
};
  ...
  try {
    ...
    if (useBrevoSMTP) {
      ...
      try {
        ...
        console.error(`[EMAIL] Brevo SMTP failed: ${smtpError.message}`);
        console.log(`[EMAIL] 🔄 Falling back to console logging (SMTP blocked in production)`);
        
        // Fallback to console logging when SMTP is blocked
